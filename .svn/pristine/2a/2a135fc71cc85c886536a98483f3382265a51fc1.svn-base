...
public class ReaderView extends AdapterView<Adapter> implements
	...
	public boolean onScale(ScaleGestureDetector detector) {
		...
		mScale = Math.min(Math.max(mScale * detector.getScaleFactor(),
				MIN_SCALE), MAX_SCALE);
	...
	private void measureView(View v) {
		...
		float scale = Math.min((float) getWidth()
				/ (float) v.getMeasuredWidth(), (float) getHeight()
				/ (float) v.getMeasuredHeight());
	...
	private Rect getScrollBounds(View v) {
		...
		return getScrollBounds(v.getLeft() + mXScroll, v.getTop() + mYScroll, v
				.getLeft()
				+ v.getMeasuredWidth() + mXScroll, v.getTop()
				+ v.getMeasuredHeight() + mYScroll);
	}

	private Point getCorrection(Rect bounds) {
		return new Point(Math.min(Math.max(0, bounds.left), bounds.right), Math
				.min(Math.max(0, bounds.top), bounds.bottom));
	}
